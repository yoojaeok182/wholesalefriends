...
public class BitmapCropTask extends AsyncTask<Void, Void, Throwable> {
    ...
    private boolean cropV2(float resizeScale) throws IOException {
        ExifInterface originalExif = new ExifInterface(mImageInputPath);

        cropOffsetX = Math.round((mCropRect.left - mCurrentImageRect.left) / resizeScale);
        cropOffsetY = Math.round((mCropRect.top - mCurrentImageRect.top) / resizeScale);
        mCroppedImageWidth = Math.round(mCropRect.width() / resizeScale);
        mCroppedImageHeight = Math.round(mCropRect.height() / resizeScale);
