...
public class TedPermissionActivity extends AppCompatActivity {
    ...
    private int nCheckPermissionShow;
    ...
    private void permissionResult(ArrayList<String> deniedPermissions, boolean isDenied) {
        Log.v(TedPermission.TAG, "permissionResult(): " + deniedPermissions);
        if (permissionListenerStack != null) {
            PermissionListener listener = permissionListenerStack.pop();
            ...
            if (ObjectUtils.isEmpty(deniedPermissions)) {
                ...
                if(isDenied){
                    listener.onPermissionFinalDenied(deniedPermissions);
                        ...
                        for (int i = 0; i < grantResults.length; i++) {
                            ...
                            if (!result) {
                                ...
                                if(ActivityCompat.shouldShowRequestPermissionRationale(this, permissions[i])) {
                                    Log.d("handa_log", "무조건 권한 허용 해야합니다.");
                                    permissionResult(deniedPermissions);
                                } else {
                                    Log.d("handa_log", "다시 묻지 않기 check");
//                                    ActivityCompat.requestPermissions(this, permissions, MY_PERMISSION_REQUEST_CODE);
    ...
    public void showPermissionDenyDialog(final ArrayList<String> deniedPermissions) {
        ...
        if (TextUtils.isEmpty(denyMessage)) {
            // denyMessage 설정 안함
            permissionResult(deniedPermissions,true);
            return;
        }
